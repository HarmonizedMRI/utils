function [gro, adc, rfCenter] = writeEPI(voxelSize, N, TE, TR, alpha, mb, pf_ky, Ry, Rz, caipiShiftZ, nFrames, nDummyFrames, type, varargin)
% function [gro, adc, rfCenter] = writeEPI(voxelSize, N, TE, TR, alpha, mb, pf_ky, Ry, Rz, caipiShiftZ, nFrames, nDummyFrames, type, varargin)
%
...
arg.rfCenter = [];                  % time to center of RF pulse (s). Default: calculate using mr.calcRfCenter()
arg.toGE = true;
arg.seqName = 'epi';
...
if isempty(arg.rfCenter)
    rfCenter = mr.calcRfCenter(rf);
else
    rfCenter = arg.rfCenter;
end
...
for ifr = (1-nDummyFrames):nFrames
    ...
    for p = IP
        rf.freqOffset = strcmp(type, 'SMS') * round((p-1)*freq);  % frequency offset (Hz) for SMS slice shift
        ...
        if arg.fatSat
            ...
            % excitation pulse
            rf.phaseOffset = rf_phase/180*pi - 2*pi*rf.freqOffset*rfCenter;  % align the phase for off-center slices
            adc.phaseOffset = rf_phase/180*pi;
        ...
        if strcmp(type, 'SMS')
            seq.addBlock(gxPre, mr.scaleGrad(gyPre, yBlipsOn), mr.scaleGrad(gzPre, zBlipsOn));
        else
            seq.addBlock(gxPre, mr.scaleGrad(gyPre, yBlipsOn), mr.scaleGrad(gzPre, p/(nz/2)*zBlipsOn*arg.gzPreOn));
        end

        if isDummyShot
            seq.addBlock(gro, ...
                         mr.scaleGrad(gyBlipUp, yBlipsOn*kyStep(1)/max(kyStepMax,1)), ...
        ...
        for ie = 2:(etl-1)
            ...
            gybd = mr.scaleGrad(gyBlipDown, yBlipsOn*kyStep(ie-1)/max(kyStepMax,1));
            gybu = mr.scaleGrad(gyBlipUp,   yBlipsOn*kyStep(ie)/max(kyStepMax,1));
        ...
        if isDummyShot
            seq.addBlock(mr.scaleGrad(gro, (-1)^(ie)), ...
                         mr.scaleGrad(gyBlipDown, yBlipsOn*kyStep(ie)/max(kyStepMax,1)), ...
